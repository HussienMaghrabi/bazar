...
class IndexController extends Controller
{
    ...
    {
//        $clients = User::where('user_type_id','2')->orderBy('id','desc')->get();
...
//        $unpaidClients = User::where('user_type_id','2')->where('paid_version',UserPaidTyps::free)->orderBy('id','desc')->get();
    ...
    {
        //onlineClients
        if($request->client_type == "onlineClients")
        {
            $onlineClients = User::where('user_type_id','2')->where('online',UserOnlineStatus::online)->orderBy('id','desc')->pluck('id');
            $this->clientsHandler($onlineClients , $request);
        }

        if($request->client_type == "paidClients")
        {
            $paidClients = User::where('user_type_id','2')->where('paid_version',UserPaidTyps::paid)->where('online',UserOnlineStatus::online)->orderBy('id','desc')->pluck('id');
            $this->clientsHandler($paidClients, $request);
        }

        if($request->client_type == "unpaidClients")
        {
            $unpaidClients = User::where('user_type_id','2')->where('paid_version',UserPaidTyps::free)->orderBy('id','desc')->pluck('id');
            $this->clientsHandler($unpaidClients, $request);
        }
