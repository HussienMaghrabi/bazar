...
@section('tbody')
    @foreach($items as $item)
        <tr>
            ...
            <td>{{$item->sub3_category['name_'.App::getLocale()]}}</td>
            <td>{{$item->sort}}</td>
            <td>{{$item->dash_created}}</td>
            <td>
                @includeIf("admin.components.buttons.edit" , ["href" => "sub4categories/$item->id/edit"])
                @includeIf("admin.components.buttons.delete",["message" => "($item->dash_name)" ,  "action" => url("admin/sub4categories/$item->id")])
            </td>
        </tr>
    @endforeach
@endsection
